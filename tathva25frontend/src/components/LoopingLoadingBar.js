...
// Loops the loading animation until isLoaded === true
export default function LoopingLoadingBar({ isLoaded }) {
  ...
  useLayoutEffect(() => {
    ...
    const setup = () => {
      ...
      const context = gsap.context(() => {
        ...
        // Ensure reset at start of each loop
        tl.set(path, {
          ...
          .to(path, { opacity: 1, duration: 0.2 }, "-=0.4")
          .to(path, { strokeDashoffset: 0, duration: 2, ease: "power2.inOut" })
          .call(() => {
            // End of one cycle. If not loaded, restart; else play exit.
