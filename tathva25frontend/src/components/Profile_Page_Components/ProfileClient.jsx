...
import {
    ...
    MdPending,
    MdCheckCircle,
    MdInfoOutline, // New icon for personal info
    MdArrowBack // Import back arrow icon
} from "react-icons/md";
...
export default function ProfileClient({ user }) {
    ...
    const [activeTab, setActiveTab] = useState("bookings"); // Default to 'bookings' for the new layout
    ...
    const [isMobileNavSticky, setIsMobileNavSticky] = useState(false); // For mobile nav
    const [openMobileSection, setOpenMobileSection] = useState('info'); // For mobile accordion
    const navRef = useRef(null); // Ref to track nav position
            ...
            return (
                <div className="space-y-5">
                    {bookingsToRender.map((booking, index) => {
                        ...
                        return (
                            <div key={booking.bookingUid} className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-gray-200">
                                <div className="flex flex-col lg:flex-row">
                                    <div className="relative w-full lg:w-48 h-32 lg:h-auto flex-shrink-0 group overflow-hidden bg-gray-100">
                                    ...
                                    <div className="flex-1 p-4">
                                        <h3 className="text-lg font-bold text-gray-900 mb-2 tracking-tight flex items-center gap-2">
                                            {booking.event?.heading} {getStatusBadge(booking?.status, "booking")}
                                        </h3>
                                        <p className="text-gray-600 text-sm mb-3 line-clamp-2 leading-relaxed">{booking.event?.description}</p>
                                        <div className="flex items-center gap-4 text-gray-700 text-sm mb-4">
                                            <div className="flex items-center gap-1">
                                                <MdDateRange size={16} className="text-yellow-500" />
                                                <span>{displayDate}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <MdAccessTime size={16} className="text-yellow-500" />
                                                <span>{displayTime}</span>
                                            </div>
                                        </div>
                                        <button onClick={() => handleDownloadTicket(booking.picture)} disabled={booking.status !== "CONFIRMED"} className="text-sm bg-black hover:bg-zinc-800 text-white px-4 py-2 rounded-lg font-bold transition-all duration-300 flex items-center justify-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                            <MdDownload size={16} />
        ...
        return (
            <div className="text-center py-10">
                <div className="bg-yellow-100 rounded-full w-20 h-20 mx-auto flex items-center justify-center mb-4">
                    <MdEvent className="text-yellow-600" size={36} />
                </div>
                <p className="text-gray-800 text-lg font-semibold mb-2">{activeTab === "bookings" ? "No confirmed events yet" : "No booking history"}</p>
                <p className="text-gray-500 text-sm max-w-xs mx-auto">Start exploring and register for exciting events!</p>
            </div>
        );
    };
    ...
    const renderReferralsList = (referralsToRender, type) => {
            ...
            return (
                <div className="space-y-5">
                    {referralsToRender.map((referral, index) => (
                        <div key={index} className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-gray-200">
                            <div className="p-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                    <div className="flex items-center gap-2 text-gray-700">
                                        <MdInfoOutline className="text-yellow-500" size={16}/>
                                        <p className="font-medium truncate">{referral.referredUser.email}</p>
                                    </div>
                                    <div className="flex items-center gap-2 text-gray-700">
                                        <MdInfoOutline className="text-yellow-500" size={16}/>
                                        <p className="font-medium">{referral.referredUser.referral}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        return (
            <div className="text-center py-10 flex flex-col items-center justify-center h-full">
                <div className="bg-yellow-100 rounded-full w-20 h-20 mx-auto flex items-center justify-center mb-4">
                    <FaUsers className="text-yellow-600" size={36} />
                </div>
    ...
    const renderAccommodationList = (bookingsToRender) => {
                            ...
                            <div key={booking.bookingUid} className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-gray-200">
                                <div className="p-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm mb-4">
                                        <div className="flex items-center gap-2 text-gray-700">
                                            <MdDateRange className="text-yellow-500" size={16} />
                                            <span>Check-in: {formatDate(booking.startDate)}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-gray-700">
                                            <MdDateRange className="text-yellow-500" size={16} />
                                            <span>Check-out: {formatDate(booking.endDate)}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-gray-700">
                                            <FaUser className="text-yellow-500" size={16} />
                                            <span className="capitalize">{booking.gender.toLowerCase()}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-gray-700">
                                            <MdEvent className="text-yellow-500" size={16} />
                                            <span className="text-sm line-clamp-1">{foodSummary}</span>
                                        </div>
                                    </div>
                                    <button onClick={() => handleDownloadTicket(booking.picture)} disabled={booking.status !== "CONFIRMED"} className="text-sm w-full bg-black hover:bg-zinc-800 text-white px-4 py-2 rounded-lg font-bold transition-all duration-300 flex items-center justify-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                        <MdDownload size={16} />
                                        {booking.status === "CONFIRMED" ? "Download Ticket" : "Failed"}
                                    </button>
                                </div>
        ...
        return (
            <div className="text-center py-10">
                <div className="bg-yellow-100 rounded-full w-20 h-20 mx-auto flex items-center justify-center mb-4"><FaBed className="text-yellow-600" size={36} /></div>
                <p className="text-gray-800 text-lg font-semibold mb-2">No accommodation booked</p>
                <p className="text-gray-500 text-sm max-w-xs mx-auto">You can book your stay through the accommodation page.</p>
            </div>
        );
    };
                    ...
                    <div key={field.key} className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                        <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                                ...
                                {editingField === field.key ? (
                                    <div className="flex items-center gap-2">
                                        <input
                                            type="text"
                                            inputMode={field.key === "phone_number" ? "numeric" : "text"}
                                            pattern={field.key === "phone_number" ? "[0-9]*" : undefined}
                                            value={tempValue}
                                            onChange={(e) => setTempValue(e.target.value)}
                                            className="flex-1 text-base font-semibold text-gray-900 border-b-2 border-yellow-500 focus:outline-none bg-transparent"
                                            autoFocus
                                            disabled={isSaving}
                                        />
                            ...
                            {field.editable && editingField !== field.key && (
                                <button onClick={() => handleEdit(field.key, currentUser[field.key])} className="ml-3 p-2 rounded-lg bg-gray-200 hover:bg-yellow-500 hover:text-black text-gray-600 transition-all"><FaEdit size={16} /></button>
                            )}
                        </div>
                    </div>
                ))}
    ...
    const renderActiveContent = () => {
        ...
        switch (activeTab) {
            ...
            case "referrals": return renderReferralsList(confirmedReferralsList, "confirmed"); // Default to confirmed referrals
    ...
    const getTabTitle = () => {
        switch (activeTab) {
            ...
            case "referrals": return "Confirmed Referrals";
            case "pendingReferrals": return "Pending Referrals";
            case "accommodation": return "My Accommodation";
    ...
    const getTabCount = () => {
        switch (activeTab) {
            ...
            case "personalInfo": return fields.length; // Count of fields for personal info
            default: return 0;
        }
    };
    ...
    return (
        <>
            ...
            {/* ======================================================================================= */}
            <div className="min-h-screen bg-gray-100 p-8 hidden lg:flex gap-8 font-sans">
                {/* Left Column (Profile + Overview) */}
                <div className="w-96 flex flex-col gap-8">

                    {/* Profile Card */}
                    <aside className="bg-black text-white p-6 flex flex-col shadow-2xl rounded-2xl">
                        <div>
                            <div className="mb-10 text-3xl font-extrabold text-yellow-500 tracking-wider">Dashboard</div>
                            {/* --- MODIFICATION: Home link added --- */}

                            <div className="flex flex-col items-center mb-10">

                                <div className="relative w-24 h-24 rounded-full overflow-hidden border-2 border-yellow-500 bg-gray-800 flex items-center justify-center mb-3">
                            ...
                            <nav className="space-y-4">
                                {[
                                    { key: "bookings", label: "My Bookings", icon: MdEvent },
                                    { key: "history", label: "Booking History", icon: MdHistory },
                                    { key: "referrals", label: "Referrals", icon: FaUsers },
                                    { key: "accommodation", label: "Accommodation", icon: FaBed },
                                ].map((item) => (
                                    <button
                                        key={item.key}
                                        onClick={() => setActiveTab(item.key)}
                                        className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-left transition-all duration-200 ${activeTab === item.key ? "bg-yellow-500 text-black font-bold shadow-md" : "text-gray-300 hover:bg-gray-800 hover:text-white"}`}
                                    >
                                        <item.icon size={20} className={activeTab === item.key ? "text-black" : "text-yellow-500"} />
                                        {item.label}
                                    </button>
                                ))}
                            </nav>
                        </div>
                        {/* --- THIS DIV WITH THE BUTTONS HAS BEEN REMOVED --- */}
                    </aside>

                {/* --- MODIFICATION: Quick Overview Card moved here --- */}
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 flex flex-col items-center justify-center">
                            <FaUsers className="text-yellow-500 mb-3" size={32}/>
                            <p className="text-3xl font-bold text-gray-900">{confirmReferrals}</p>
                            <p className="text-sm text-gray-500 mt-1">Confirmed Referrals</p>
                        </div>
                        {confirmedBookings.length > 0 && (
                            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 flex flex-col items-center justify-center">
                                <MdEvent className="text-yellow-500 mb-3" size={32}/>
            ...
            <main className="flex-1 flex flex-col bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                <div className="p-8 pb-0 border-b border-gray-200">
                    ...
                    {(activeTab === "referrals" || activeTab === "pendingReferrals") && (
                        <div className="flex mt-6 space-x-6">
                            <button
                                onClick={() => setActiveTab("referrals")}
                                className={`py-3 px-1 text-sm font-semibold transition-all duration-200 ${
                                    activeTab === "referrals"
            ...
            {/* --- MODIFICATION: Right Panel is now a container for multiple cards --- */}
                {/* --- MODIFICATION: Right Panel is now a container for the two cards --- */}
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
                        ...
                        <div className="space-y-4">
                            <button
                                ...
                                className={`w-full ${copied ? "bg-green-500" : "bg-yellow-500 hover:bg-yellow-600"} text-black py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all duration-300 shadow-lg`}
                            >
                                {copied ? <><FaCheck size={16} /> Copied!</> : <><FaShare size={16} /> Refer a Friend</>}
                            </button>
                            <button
                                onClick={() => { localStorage.removeItem("jwt"); router.push("/"); }}
                                className="w-full bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold transition-colors duration-200 py-3 rounded-lg flex items-center justify-center gap-2"
                            >
                                Logout
                            </button>
                        </div>
            ...
            {/* ======================================================================================= */}
            {/* MOBILE LAYOUT - Hidden on screens lg and larger */}
            {/* ======================================================================================= */}
            <div className="lg:hidden min-h-screen bg-gray-100 font-sans">
                {/* User Header Card */}
                <div className="bg-black text-white p-6 shadow-lg">
                    <div className="flex items-center gap-4">
                        <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-yellow-500 bg-gray-800 flex-shrink-0">
                            <Image src={currentUser.picture} alt="user_pfp" fill className="object-cover" />
                        </div>
                        <div>
                            <h1 className="text-xl font-bold text-white">{currentUser.name}</h1>
                            <p className="text-sm font-mono text-gray-400">{currentUser.tat_id}</p>
                        </div>
                    </div>
                    <div className="flex gap-3 mt-4">
                        <button onClick={handleCopyReferral} className={`flex-1 ${copied ? "bg-green-500" : "bg-yellow-500 hover:bg-yellow-600"} text-black py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all`}>
                            {copied ? <><FaCheck size={14} /> Copied!</> : <><FaShare size={14} /> Refer</>}
                        </button>
                        <button onClick={() => { localStorage.removeItem("jwt"); router.push("/"); }} className="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all">
                            Logout
                        </button>
                    </div>
                </div>
                ...
                {/* Sticky Navigation */}
                <nav ref={navRef} className={`sticky top-0 z-20 bg-white shadow-md ${isMobileNavSticky ? 'shadow-lg' : ''}`}>
                    <div className="flex justify-around border-b border-gray-200">
                        {[
                            { key: "bookings", label: "Bookings", icon: MdEvent },
                            { key: "referrals", label: "Referrals", icon: FaUsers },
                            { key: "accommodation", label: "Stay", icon: FaBed },
                            { key: "info", label: "My Info", icon: MdInfoOutline }
                        ].map(item => (
                            <button key={item.key} onClick={() => setActiveTab(item.key)} className={`flex-1 flex flex-col items-center gap-1 py-3 text-xs font-semibold transition-colors ${activeTab === item.key ? 'text-yellow-600 border-b-2 border-yellow-600' : 'text-gray-500'}`}>
                                <item.icon size={20} />
                ...
                {/* Mobile Content */}
                <div className="p-4">
                    {/* Render content based on activeTab, but Info tab has special rendering */}
                    ...
                    {activeTab === 'info' && (
                        <div className="space-y-4">
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                <button onClick={() => setOpenMobileSection(openMobileSection === 'info' ? null : 'info')} className="w-full flex justify-between items-center p-4">
                                    <h2 className="font-bold text-gray-900">Personal Information</h2>
                                    {/* Chevron Icon */}
                                </button>
                                {openMobileSection === 'info' && <div className="p-4 border-t border-gray-200">{renderPersonalInfo()}</div>}
                            </div>
                            {/* Accordion for Quick Overview */}
                            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                <button onClick={() => setOpenMobileSection(openMobileSection === 'overview' ? null : 'overview')} className="w-full flex justify-between items-center p-4">
                                    <h2 className="font-bold text-gray-900">Quick Overview</h2>
                                    {/* Chevron Icon */}
                                </button>
                                {openMobileSection === 'overview' && (
                                    <div className="p-4 border-t border-gray-200 space-y-4">
                                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="text-2xl font-bold text-gray-900">{confirmReferrals}</p>
                                                <p className="text-sm text-gray-500">Confirmed Referrals</p>
                                            </div>
                                        </div>
                                        {confirmedBookings.length > 0 && (
                                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 flex items-center gap-4">
